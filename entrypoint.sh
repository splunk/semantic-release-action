...
#gpg --default-key E7A43792E68AD642 --yes --pinentry-mode loopback --passphrase $passphrase_decoded --clearsign testfile.txt
echo '/usr/bin/gpg --default-key E7A43792E68AD642 --passphrase "$passphrase_decoded" --verbose --pinentry-mode loopback "$@"' > /tmp/gpg-with-passphrase && chmod +x /tmp/gpg-with-passphrase

echo "========DEBUG=========="
gpg --status-fd=2 -bsau "E7A43792E68AD642"
which gpg
gpg --version
cat ~/.gitconfig
export GIT_TRACE=1
...
git config --global user.signingkey "E7A43792E68AD642"
